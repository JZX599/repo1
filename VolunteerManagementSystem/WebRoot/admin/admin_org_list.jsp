...
<c:if test="${empty admin }">
		...
		<div class="son_body" style="margin-top: -20px">
			...
			<div class="row" style="margin: 0;padding: 5px;">
				...
				<div class="col-md-3">
					...
					<div class="col-md-9">
						<form class="navbar-form  " action="${ctx }/AdminServlet?action=getAllOrgList" method="post">
				...
				<div class="row" style="margin: 0;padding: 0;">
					<table class="table  table-bordered table-hover table-condensed">
						...
						<c:if test="${fn:length(OrgpageList)==0 }">
							<tr>
								<td colspan="10" style="font-weight: 600;">暂无组织信息</td>
							</tr>
						</c:if>
						<c:if test="${fn:length(OrgpageList)>0 }">
						...
						<tr>
								...
								<!-- 组织状态0待审核1已通过2限制登录 -->
								<c:if test="${org.OState==0 }">
								<td>待审核</td>
								<td><a href="${ctx }/AdminServlet?action=passOrgregist&OId=${org.OId }&flag=all" onclick="return confirm('通过该组织注册？');" class="btn btn-pass btn-xs">通过</a>
									<a href="${ctx }/AdminServlet?action=notpassOrgregist&OId=${org.OId }&flag=all" onclick="return confirm('拒绝该组织注册？');" class="btn btn-danger btn-xs">拒绝</a>
								</td>
								...
								<td><a href="${ctx }/AdminServlet?action=ResetOrgPwd&OId=${org.OId }&flag=all" onclick="return confirm('要重置该组织的密码？');" class="btn btn-success btn-xs">重置密码</a>
									<a href="${ctx }/AdminServlet?action=DongJOrg&OId=${org.OId }&flag=all" onclick="return confirm('要冻结该组织？');" class="btn btn-danger btn-xs">冻结</a>
								...
								<td>冻结</td>
								<td><a href="${ctx }/AdminServlet?action=JieDOrg&OId=${org.OId }&flag=all" onclick="return confirm('要解除冻结该组织？');" class="btn btn-danger btn-xs">解冻</a>
								</td>
								</c:if>
							</tr>
						</c:forEach>
	...
	<script type="text/javascript">
		$(function() {
			...
			$(window).attr('location',"${ctx }/AdminServlet?action=getAllOrgList&type=${type}&page="+page);
